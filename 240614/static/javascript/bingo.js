...
function bingoCheck(){
    ...
    var idx = $(".num").index(this);  // 클릭한 td가 몇 번째 인덱스인가
    bingo[idx] = 0;  // 해당 td 위치와 같은 bingo 계열에 0으로 변경
    ...
    */
    ...
    function endGame(){
        $(".num").off();  // 모든 이벤트 제거  .off(###) 특정 이벤트 ### 제거
...
function draw(){  // 배열의 값 테이블(td)에 출력
    ...
    for( var i=0; i<td.length; i++ ){
        td.eq(i).text(bingo[i]);  // td[i].text(bingo[i]);
    }
}